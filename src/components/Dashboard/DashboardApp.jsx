...
const chatMessages = [
  { sender: "support", text: "Your order is being prepared", time: "2m ago" },
  { sender: "me", text: "Perfect, please keep it hot.", time: "1m ago" },
  { sender: "support", text: "Rider is on the way", time: "now" },
];
...
function DashboardApp() {
  ...
  return (
    <div className="mx-auto max-w-[1260px] px-4 py-10 sm:px-6 lg:px-8">
      <div className="grid overflow-hidden rounded-[30px] border border-[#1F1B14]/[0.08] bg-white shadow-[0_40px_80px_-30px_rgba(150,100,20,0.35)] md:grid-cols-[210px_1fr_260px]">
            ...
            {dishes.map(({ name, price, discount, image }) => (
              ...
              >
                ...
                <img
                  src={image}
                  alt={name}
                  className="h-[74px] w-full rounded-[10px] object-cover"
                />
        ...
        <aside className="border-t border-[#1F1B14]/[0.08] bg-white p-5 md:border-l md:border-t-0">
          ...
          {chatOpen && (
            <div className="mt-5 rounded-[18px] border border-[#1F1B14]/[0.08] bg-[#FFF9EC] p-3 shadow-[0_20px_25px_-20px_rgba(150,100,20,0.25)]">
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#F5A623]/15 text-[#F08B1D]">
                    <MessageCircle size={15} strokeWidth={2.2} />
                  </div>
                  <div>
                    <div className="text-[12px] font-bold text-[#1F1B14]">
                      GoMeal Support
                    </div>
                    <div className="text-[9.5px] text-[#8A8478]">
                      Online now
                    </div>
                  </div>
                </div>
                <button
                  ...
                  className="text-[#8A8478] transition hover:text-[#1F1B14]"
                >
              ...
              <div className="space-y-2 text-[11px]">
                {chatMessages.map((message, index) => (
                  ...
                  >
                    <div
                      className={`max-w-[75%] rounded-2xl px-2.5 py-2 ${message.sender === "me" ? "bg-[#F5A623] text-white" : "bg-white text-[#1F1B14]"} shadow-sm`}
                    >
                      {message.text}
                      <div
                        className={`mt-1 text-[8.5px] ${message.sender === "me" ? "text-white/80" : "text-[#8A8478]"}`}
                      >
                        {message.time}
                      </div>
                    </div>
              ...
              <div className="mt-3 flex items-center gap-2 rounded-full border border-[#1F1B14]/[0.08] bg-white px-2 py-1.5">
                <input
                  type="text"
                  placeholder="Type a message"
                  className="w-full bg-transparent px-2 py-1 text-[11px] text-[#1F1B14] placeholder:text-[#8A8478] outline-none"
                />
                <button
                  type="button"
                  aria-label="Send message"
                  className="flex h-7 w-7 items-center justify-center rounded-full bg-[#F5A623] text-white transition hover:bg-[#F08B1D]"
                >
